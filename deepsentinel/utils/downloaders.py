    ...
    for idx, pt in pts.iterrows():
        ...
        try:
            ...
            if 'local' in destinations:
                name_root = os.path.join(os.getcwd(),'data',version,str(idx),'_'.join([str(idx),'OSMFC',pt['DL_S2'].split(':')[2][0:10], str(pt['lon']), str(pt['lat'])]))

            else:
                name_root = os.path.join(os.getcwd(),'data','tmp','_'.join([str(idx),'OSMFC',pt['DL_S2'].split(':')[2][0:10], str(pt['lon']), str(pt['lat'])]))
...
def DL_CLC_downloader(version, pts, ii_ps, CONFIG, mp_idx, TLs, destinations):
    ...
    for idx, pt in pts.iterrows():
        ...
        try:
            ...
            if 'local' in destinations:
                name_root = os.path.join(os.getcwd(),'data',version,str(idx),'_'.join([str(idx),'LC',pt['DL_S2'].split(':')[2][0:10], str(pt['lon']), str(pt['lat'])]))

            else:
                name_root = os.path.join(os.getcwd(),'data','tmp','_'.join([str(idx),'LC',pt['DL_S2'].split(':')[2][0:10], str(pt['lon']), str(pt['lat'])]))
